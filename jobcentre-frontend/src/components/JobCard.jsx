import { Bookmark, Briefcase, Building2, Clock3, MapPin, Share2, UsersRound } from "lucide-react";
...
export default function JobCard({job}) {
  ...
  return <article className="job-card">
    <header className="job-author"><div className="company-logo">{avatar && !avatarFailed?<img src={avatar} alt={`${job.organisation_name || "Employer"} profile picture`} loading="lazy" onError={() => setAvatarFailed(true)}/>:<Building2/>}</div><div><b>{job.organisation_name}</b>{job.employer_phone_verified && <span className="verified">✓ Phone verified</span>}<small>Posted {new Date(job.created_at).toLocaleDateString("en-ZA")}</small></div>{job.urgent && <span className="urgent">Urgent</span>}</header>
