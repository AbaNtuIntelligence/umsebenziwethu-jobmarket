...
export default function JobCard({job}) {
  ...
  return <article className="job-card">
    <header className="job-author"><div className="company-logo">{job.employer_avatar?<img src={job.employer_avatar} alt=""/>:<Building2/>}</div><div><b>{job.organisation_name}</b>{job.employer_verified && <span className="verified">✓ Verified</span>}<small>Posted {new Date(job.created_at).toLocaleDateString("en-ZA")}</small></div>{job.urgent && <span className="urgent">Urgent</span>}</header>
